...
async function initDb() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS feedback (id INTEGER PRIMARY KEY AUTOINCREMENT, customer_name TEXT NOT NULL, customer_phone TEXT, product_id INTEGER, product_name TEXT, rating INTEGER NOT NULL CHECK(rating BETWEEN 1 AND 5), comment TEXT NOT NULL, status TEXT DEFAULT 'pending', created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (product_id) REFERENCES products(id));
    CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, customer_name TEXT NOT NULL, customer_phone TEXT, customer_email TEXT, subject TEXT NOT NULL, message TEXT NOT NULL, status TEXT DEFAULT 'unread', replied_by TEXT, reply TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP);
  `);
  // Role migration — add role column to admins if doesn't exist
  try { db.exec("ALTER TABLE admins ADD COLUMN role TEXT DEFAULT 'admin'"); } catch { /* already exists */ }
